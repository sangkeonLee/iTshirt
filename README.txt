...
4강 둘 이상의 원격 저장소로 협업하기 - 원격저장소 복사(fork)
    github의 settings > Collaborators 메뉴에서 다른 github이용자가 커밋을 할 수 있게 추가할수 있다.
    5명 정도의 소규모 개발자가 협업을 한다면 Collaborators추가하여 브랜치를 나누어 작업하는게 효율적이다.
    하지만 사람이 많아지면 fork로 직접 원격저장소를 복사해 개발하는게 좋다.(원본 저장소에 영향을 미치지 않기 때문에.)
    다만 원본 저장소의 이력(history)을 보려면 따로 주소를 추가해야 한다.

    leesk928+2@gmail.com 계정 생성(name SangkeonLee2)